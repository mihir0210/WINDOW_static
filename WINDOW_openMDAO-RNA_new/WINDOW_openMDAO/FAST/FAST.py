...
from WINDOW_openMDAO.FAST.airfoils_rna_copy import ReadAirfoil
from WINDOW_openMDAO.FAST.load_case_selector import load_case_selector
from WINDOW_openMDAO.FAST.post_processor import post_processor
...
class FAST(AbsFAST):
    ...
    def compute(self,inputs,outputs):
        ...
        blade_ac[np.where(np.isnan(blade_ac))]= np.interp(blade_radius[np.where(np.isnan(blade_ac))][:,0], blade_radius[np.where(~np.isnan(blade_ac))][:,0], blade_ac[np.where(~np.isnan(blade_ac))])
        ...
        scipy.io.savemat('Blade.mat', dict(Blade_Mass=blade_mass, Blade_EIflap=blade_ei_flap, Blade_EIedge=blade_ei_edge,
        ...
        airfoil_cnslope = np.array([0, 0, 7.48880, 7.18380, 7.33260, 6.44620, 6.20470, 6.00310]).reshape(num_airfoils,1)
        ...
        airfoil_critcn2 = np.array([0, 0, -0.3226, -0.3075, -0.6138, -0.6873, -0.5324, -0.7945]).reshape(num_airfoils,1)

        scipy.io.savemat('Airfoil.mat', dict(Airfoil_Geometry1=airfoil_geometry_cyl1,
                                             Airfoil_Geometry2=airfoil_geometry_cyl2, Airfoil_Geometry3=airfoil_geometry_du405,
                                             Airfoil_Geometry4=airfoil_geometry_du350, Airfoil_Geometry5=airfoil_geometry_du300,
                                             Airfoil_Geometry6=airfoil_geometry_du250, Airfoil_Geometry7=airfoil_geometry_du210,
                                             Airfoil_Geometry8=airfoil_geometry_naca, Airfoil_Alpha1=cyl1_alpha, Airfoil_Alpha2=cyl2_alpha,
                                             Airfoil_Alpha3=du405_alpha, Airfoil_Alpha4=du350_alpha, Airfoil_Alpha5=du300_alpha,
                                             Airfoil_Alpha6=du250_alpha, Airfoil_Alpha7=du210_alpha, Airfoil_Alpha8=naca_alpha,
                                             Airfoil_Cl1=cyl1_cl, Airfoil_Cl2=cyl2_cl, Airfoil_Cl3=du405_cl, Airfoil_Cl4=du350_cl,
                                             Airfoil_Cl5=du300_cl, Airfoil_Cl6=du250_cl, Airfoil_Cl7=du210_cl, Airfoil_Cl8=naca_cl,
                                             Airfoil_Cd1=cyl1_cd, Airfoil_Cd2=cyl2_cd, Airfoil_Cd3=du405_cd, Airfoil_Cd4=du350_cd,
                                             Airfoil_Cd5=du300_cd, Airfoil_Cd6=du250_cd, Airfoil_Cd7=du210_cd, Airfoil_Cd8=naca_cd,
                                             Airfoil_Cm1=cyl1_cm, Airfoil_Cm2=cyl2_cm, Airfoil_Cm3=du405_cm, Airfoil_Cm4=du350_cm,
                                             Airfoil_Cm5=du300_cm, Airfoil_Cm6=du250_cm, Airfoil_Cm7=du210_cm, Airfoil_Cm8=naca_cm,
                                             Airfoil_CnSlope=airfoil_cnslope, Airfoil_StallAngle1=airfoil_stallang1, Airfoil_StallAngle2=airfoil_stallang2,
        ...
        tower_wallthickness=np.array([[np.linspace(tower_bthickness,tower_tthickness,len(tower_height))]]).reshape(num_tnodes,1) #transpose
        tower_mass = tower_density*math.pi*(tower_diameter**2 - (tower_diameter - 2*tower_wallthickness)**2)/4 + tower_extramass
        ...
        scipy.io.savemat('Tower.mat', dict(Tower_HubHeight=tower_hh, Tower_BottomThickness=tower_bthickness, Tower_TopThickness=tower_tthickness,
                                           Tower_Height=tower_height, Tower_ExtraMass=tower_extramass, Tower_Diameter=tower_diameter,
                                           Tower_WallThickness=tower_wallthickness, Tower_Mass=tower_mass, Tower_EI=tower_ei,
        ...
        scipy.io.savemat('Nacelle.mat', dict(Nacelle_Hub_Length=nacelle_hub_length, Nacelle_Hub_Mass=nacelle_hub_mass,
                                             Nacelle_Hub_Overhang=nacelle_hub_overhang, Nacelle_Hub_Type=nacelle_hub_type,
                                             Nacelle_Hub_ShaftTilt=nacelle_hub_shafttilt, Nacelle_Housing_Type=nacelle_housing_type,
                                             Nacelle_Housing_Diameter=nacelle_housing_diameter, Nacelle_Housing_Length=nacelle_housing_length,
                                             Nacelle_Housing_Mass=nacelle_housing_mass))



        #DriveTrain and Controls inputs
        rated_power=inputs['rated_power'] #in W
        ...
        output = eng.GenerateSteadyOp(fine_pitch, nargout=2)
        Cp=output[0]
        tsr=output[1]
        rated_ws=(2*rated_power/(Cp*rho_air*rotor_area))**(1.0/3.0)


        control_windspeed_cutin=4.0
        control_windspeed_cutout=25.0

        cutin_omega_lss=tsr*control_windspeed_cutin/blade_radius[-1] #in radian/s
        ...
        rated_omega_lss=tsr*rated_ws/blade_radius[-1] #in radian/s
        ...
        opt_mode_gain=0.5*1.225*Cp*math.pi*drivetrain_gear_eff*blade_radius[-1]**5/(tsr**3)/(drivetrain_gear_ratio**3)
        opt_mode_gain_rpm = opt_mode_gain * 4 * math.pi ** 2 / 60 / 60  # converting to /rpm2
        mech_power=rated_power/drivetrain_gen_eff #in W
        ...
        drivetrain_gen_hssinertia = 534.116 * (rated_power / (5*10**6)) ** (5 / 3) * (12.1 * 97 / control_omega_c) ** 2
        drivetrain_lssinertia=drivetrain_gen_hssinertia*drivetrain_gear_ratio**2

        scipy.io.savemat('Drivetrain.mat', dict(Drivetrain_Generator_Efficiency=drivetrain_gen_eff,
        ...
        scipy.io.savemat('Control.mat', dict(Control_WindSpeed_Cutin=control_windspeed_cutin,
        ...
        gains = eng.Control_gains(rated_ws, rated_omega_lss_rpm, nargout=2)
        ...
        #run load case 1.3
        load_case_selector(1.3,rotor_diameter)

        ret= eng.Certification(nargout=0)
        [Root_stress_13, Deflection_13, Stress_tower_base_13, Stress_tower_base_shear_13,_,Tensile_stress_bolt_13]=\
        post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,blade_mass_total,
                       blade_radius[-1],airfoil_geometry_cyl1,1,0)


        #Stresses in Pa and Tip deflection (m)
        print 'DLC 1.3, Root stress in MPa:', Root_stress_13
        print 'DLC 1.3, Deflection in MPa:', Deflection_13
        print 'DLC 1.3, Stress_tower_base in MPa:', Stress_tower_base_13
        print 'DLC 1.3, Stress_tower_base_shear in MPa:', Stress_tower_base_shear_13
        print 'DLC 1.3, Tensile stress in the bolts in MPa:', Tensile_stress_bolt_13

        '''
        #run load case 2.1
        load_case_selector(2.1,rotor_diameter)
        ret= eng.Certification(nargout=0)
        [Root_stress_21, Deflection_21, Stress_tower_base_21, Stress_tower_base_shear_21,_]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1,1,0)


        #Stresses in Pa and Tip deflection (m)
        print 'DLC 2.1, Root stress in MPa:', Root_stress_21
        print 'DLC 2.1, Deflection in MPa:', Deflection_21
        print 'DLC 2.1, Stress_tower_base in MPa:', Stress_tower_base_21
        print 'DLC 2.1, Stress_tower_base_shear in MPa:', Stress_tower_base_shear_21

        #run load case 3.3
        load_case_selector(3.3,rotor_diameter)
        ret= eng.Certification(nargout=0)
        [Root_stress_33, Deflection_33, Stress_tower_base_33, Stress_tower_base_shear_33,_]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1,1,0)

        #Stresses in Pa and Tip deflection (m)
        print 'DLC 3.3, Root stress in MPa:', Root_stress_33
        print 'DLC 3.3, Deflection in MPa:', Deflection_33
        print 'DLC 3.3, Stress_tower_base in MPa:', Stress_tower_base_33
        print 'DLC 3.3, Stress_tower_base_shear in MPa:', Stress_tower_base_shear_33


        #### Run FATIGUE analysis and obtain Fatigue damage and damage equivalent load###
        load_case_selector('Fatigue',rotor_diameter)
        ret= eng.Certification(nargout=0)

        variables_1 = scipy.io.loadmat('DLC_U=6.00.mat')
        variables_2 = scipy.io.loadmat('DLC_U=8.00.mat')
        variables_3 = scipy.io.loadmat('DLC_U=12.00.mat')
        variables_4 = scipy.io.loadmat('DLC_U=16.00.mat')
        variables_5 = scipy.io.loadmat('DLC_U=20.00.mat')
        variables_6 = scipy.io.loadmat('DLC_U=23.50.mat')
        


        [_, _, _, _,Stress_ts_6]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_1)
        [_, _, _, _,Stress_ts_8]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_2)
        [_, _, _, _,Stress_ts_12]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_3)
        [_, _, _, _,Stress_ts_16]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_4)
        [_, _, _, _,Stress_ts_20]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_5)
        [_, _, _, _,Stress_ts_23_5]=post_processor(tower_bthickness, tower_diameter, blade_ei_flap,blade_chord,airfoil_geometry_cyl1, 2, variables_6)


        Stress_ts=np.vstack([Stress_ts_6, Stress_ts_8, Stress_ts_12, Stress_ts_16, Stress_ts_20, Stress_ts_23_5])
        scipy.io.savemat('Stress.mat', dict(Stress_ts=Stress_ts))
        Results = eng.Fatigue_calculator(nargout=2)
        Fatigue=Results[0]
...
if __name__ == "__main__":
    ...
    inputs = {'blade_number':3.0, 'blade_ifoil': [0, 1, 2, 3, 4, 5, 6, 7],'blade_cone' :2.5,
              ...
              'blade_ei_flap':[1.8*10**10, 1.2*10**10, 4.407*10**9, 2.15*10**9, 1.38*10**9, 6.37*10**8, 2.65*10**8,
                               1.07*10**8, 5.69*10**7, 4.09*10**6],
              'blade_ei_edge':[1.8*10**10, 1.64*10**10, 7.23*10**9, 4.63*10**9, 3.77*10**9, 2.68*10**9,

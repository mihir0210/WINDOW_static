...
class WorkingGroup(Group):
    ...
    def setup(self):
        indep2 = self.add_subsystem('indep2', IndepVarComp(), promotes_outputs=['design_tsr', 'chord_coefficients', 'twist_coefficients',
                                                                                'pitch', 'tau'])
        ...
        indep2.add_output('drive_train_efficiency', desc='efficiency of aerodynamic to electrical conversion', val=0.944)
        ...
        #indep2.add_output('tower_tthickness')
        #indep2.add_output('tower_height',shape=self.num_tnodes)
        indep2.add_output('tower_extramass',val=np.array([329.0, 308.0, 287.0, 268.0, 249.0, 230.0, 213.0, 196.0, 179.0, 164.0, 149.0]))
        #indep2.add_output('tower_diameter', shape=self.num_tnodes)
        ...
        indep2.add_output('chord_coefficients', units='m', desc='coefficients of polynomial chord profile',
                          val = np.array([0.75 , 0.92419355, 0.76451613]))
        indep2.add_output('twist_coefficients', units='deg', desc='coefficients of polynomial twist profile',
                          val= np.array([1.3, 0.93225806, 1.08709677]))
        indep2.add_output('pitch', units='deg', desc='pitch angle', val=0.3364129)
        #indep2.add_output('tau_root', val=0.8)
        #indep2.add_output('tau_75', val=0.8)
        indep2.add_output('design_tsr', desc='design tip speed ratio', val=1.02467742)
        indep2.add_output('tau', val=0.7)
        ...
        self.add_subsystem('obj', ExecComp('f=lcoe/8.35'))  # Reference value
        ...
        self.connect('FAST.Tip_Deflection', 'c1.deflection')

        self.add_subsystem('c2', ExecComp('Spar_stress=flapwise_stress_spar/1000'))  # Reference value
        self.connect('FAST.Flapwise_Stress_Spar', 'c2.flapwise_stress_spar')'''
        ...
        self.connect('FAST.Max_Stress_Spar', 'c2.max_stress_spar')

        self.add_subsystem('c3', ExecComp('Skin_stress=max_stress_skin/700'))  # Reference value
        self.connect('FAST.Max_Stress_Skin', 'c3.max_stress_skin')

        self.add_subsystem('c4', ExecComp('Te_Reinf_stress=max_stress_te_reinf/702'))  # Reference value
        self.connect('FAST.Max_Stress_Te_Reinf', 'c4.max_stress_te_reinf')

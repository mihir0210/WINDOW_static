...
def span_layup(span_radius, span_section, web, skin, root, le_core1, spar1, te_core1, te_reinf1, le_core2, spar2,
    ...
    section = np.array([0, 0.15, 0.3, 0.5, 0.75, 0.95, 1]).reshape(1,7).astype(float)
    ...
    le_core = np.array([0, le_core1, le_core2, le_core3, le_core4, 0, 0]).reshape(1,7).astype(float)
    te_core = np.array([0, te_core1, te_core2, te_core3, te_core4, 0, 0]).reshape(1,7).astype(float)
    spar = np.array([0, spar1, spar2, spar3, spar4, 0, 0]).reshape(1,7).astype(float)
    te_reinf = np.array([0, te_reinf1, te_reinf2, te_reinf3, 0, 0, 0]).reshape(1,7).astype(float)
    root = np.array([root, 0, 0, 0, 0, 0, 0]).reshape(1,7).astype(float)
    skin = np.array([skin, skin, skin, skin, skin, skin, skin]).reshape(1,7).astype(float)
    ...
    root_span = np.interp(span_section[0,:], section[0,:], root[0,:])
    ...
    scipy.io.savemat('Matlab_scripts\Internal_layup.mat', dict(skin_span=skin_span, root_span=root_span,

...
#### Add Scipy to save span Chord and Span Airfoil mat file in Preprocessor Folder ####
import scipy.io
...
class PeggedNodes(AbsAerodynamicDesign):   
    ...
    def compute(self, inputs, outputs):
        ...
        #chord_ref = np.array([3.542, 3.01, 2.313])
        #twist_ref = np.array([13.308, 9.0, 3.125])

        #chord_coefficients = np.multiply(chord_coefficients, chord_ref)
        #twist_coefficients = np.multiply(twist_coefficients, twist_ref)

        #print chord_coefficients, twist_coefficients
        ...
        scipy.io.savemat('random', dict(Chord_profile = np.array(span_chord), Twist_profile=np.array(span_twist),
                                                Blade_radius = np.array(span_r)))

        span_r = np.ndarray.flatten(span_r)
        span_chord = np.asarray(span_chord)
        span_twist = np.asarray(span_twist)
        span_chord = np.ndarray.flatten(span_chord)
        span_twist = np.ndarray.flatten(span_twist)

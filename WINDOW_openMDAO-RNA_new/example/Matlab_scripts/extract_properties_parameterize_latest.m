function [EI_flap, EI_edge, G_stiffness, EA, M_L, y_flapwise_spar, y_flapwise_skin,...
         y_edgewise_te_reinf,...
        y_edgewise_skin,rel_xe_section, rel_ye_section, E] = extract_properties_parameterize_latest(location, root, skin, le_core, spar, te_core, te_reinf, web)
...
difference=abs(location-Blade.Radius);
I=find(difference==min(difference)); %index for Blade chord to be used

%section=Blade.Radius(I)/61.5;  % change to blade radius
%section=Blade.Radius(I)/83.5;  % change to blade radius
%section=Blade.Radius(I)/100;
...
%airfoil_transition_index(find(airfoil_transition_index<I,1,'last'));
...
delta_x =Blade.Radius(I) - Blade.Radius(airfoil_transition_index(find(airfoil_transition_index<=I,1,'last')));
...
previous_airfoilno = Blade.NFoil(airfoil_transition_index(find(airfoil_transition_index<=I,1,'last')));

% Find the previous airfoil ID where transition occured
% slope of that number is the slope of that section 

airfoil = [Airfoil.Geometry{1,previous_airfoilno}(1,:);(Airfoil.Geometry{1,previous_airfoilno}(2,:) + slope(previous_airfoilno,:)*delta_x)];
...
difference=abs(airfoil_upper_x-le_panel_end_);
I_le_panel_end=find(difference==min(difference));
...
theta_segment_le_upper(i) = atand((airfoil_upper_y_abs(i+1)-airfoil_upper_y_abs(i))./(airfoil_upper_x_abs(i+1)-airfoil_upper_x_abs(i)));
theta_segment_le_lower(i) = atand((airfoil_lower_y_abs(i+1)-airfoil_lower_y_abs(i))./(airfoil_lower_x_abs(i+1)-airfoil_lower_x_abs(i)));

% if theta_segment_le_upper(i)<0
%     theta_segment_le_upper(i)=360 + theta_segment_le_upper(i);
% end
% 
% if theta_segment_le_lower(i)<0
%     theta_segment_le_lower(i) = 360+ theta_segment_le_upper(i);
% end


I_xx_upper_le(i)=length_element_upper_le(i)*t_eq_le_panel^3/12; %%% As all elements are rectangles, Ixy about centroid would be zero
I_xx_lower_le(i)=length_element_lower_le(i)*t_eq_le_panel^3/12;

I_yy_upper_le(i)=t_eq_le_panel*length_element_upper_le(i).^3/12;
I_yy_lower_le(i)=t_eq_le_panel*length_element_lower_le(i).^3/12;

%%%% Rotate co-ordinate system to make it parallel to chord line axis %%%%
I_xx_own_le_upper(i) = (I_xx_upper_le(i) + I_yy_upper_le(i))/2 + (I_xx_upper_le(i)-I_yy_upper_le(i)).*cosd(2*theta_segment_le_upper(i))/2;
I_xx_own_le_lower(i) = (I_xx_lower_le(i) + I_yy_lower_le(i))/2 + (I_xx_upper_le(i)-I_yy_lower_le(i)).*cosd(2*theta_segment_le_lower(i))/2;

I_yy_own_le_upper(i) = (I_xx_upper_le(i) + I_yy_upper_le(i))/2 - (I_xx_upper_le(i)-I_yy_upper_le(i)).*cosd(2*theta_segment_le_upper(i))/2;
I_yy_own_le_lower(i) = (I_xx_lower_le(i) + I_yy_lower_le(i))/2 - (I_xx_lower_le(i)-I_yy_lower_le(i)).*cosd(2*theta_segment_le_lower(i))/2;
...
difference=abs(airfoil_upper_x-spar_cap_end_);
I_spar_cap_end=find(difference==min(difference));
...
theta_segment_sc_upper(j) = atand((airfoil_upper_y_abs(i+1)-airfoil_upper_y_abs(i))./(airfoil_upper_x_abs(i+1)-airfoil_upper_x_abs(i)));
theta_segment_sc_lower(j) = atand((airfoil_lower_y_abs(i+1)-airfoil_lower_y_abs(i))./(airfoil_lower_x_abs(i+1)-airfoil_lower_x_abs(i)));

% if theta_segment_sc_upper(j)<0
%     theta_segment_sc_upper(j)=360 + theta_segment_sc_upper(j);
% end
% 
% if theta_segment_sc_lower(j)<0
%     theta_segment_sc_lower(j) = 360 + theta_segment_sc_lower(j);
% end


I_xx_upper_sc(j)=length_element_upper_sc(j)*t_eq_sc^3/12; %%% As all elements are rectangles, Ixy about centroid would be zero
I_xx_lower_sc(j)=length_element_lower_sc(j)*t_eq_sc^3/12;

I_yy_upper_sc(j)=t_eq_sc*length_element_upper_sc(j).^3/12;
I_yy_lower_sc(j)=t_eq_sc*length_element_lower_sc(j).^3/12;


%%%% Rotate co-ordinate system to make it parallel to chord line axis %%%%
I_xx_own_sc_upper(j) = (I_xx_upper_sc(j) + I_yy_upper_sc(j))/2 + (I_xx_upper_sc(j)-I_yy_upper_sc(j)).*cosd(2*theta_segment_sc_upper(j))/2;
I_xx_own_sc_lower(j) = (I_xx_lower_sc(j) + I_yy_lower_sc(j))/2 + (I_xx_upper_sc(j)-I_yy_lower_sc(j)).*cosd(2*theta_segment_sc_lower(j))/2;

I_yy_own_sc_upper(j) = (I_xx_upper_sc(j) + I_yy_upper_sc(j))/2 - (I_xx_upper_sc(j)-I_yy_upper_sc(j)).*cosd(2*theta_segment_sc_upper(j))/2;
I_yy_own_sc_lower(j) = (I_xx_lower_sc(j) + I_yy_lower_sc(j))/2 - (I_xx_lower_sc(j)-I_yy_lower_sc(j)).*cosd(2*theta_segment_sc_lower(j))/2;
...
difference=abs(airfoil_upper_x-te_reinf_start_);
I_te_reinf_start=find(difference==min(difference));
...
theta_segment_te_reinf_upper(j) = atand((airfoil_upper_y_abs(i+1)-airfoil_upper_y_abs(i))./(airfoil_upper_x_abs(i+1)-airfoil_upper_x_abs(i)));
theta_segment_te_reinf_lower(j) = atand((airfoil_lower_y_abs(i+1)-airfoil_lower_y_abs(i))./(airfoil_lower_x_abs(i+1)-airfoil_lower_x_abs(i)));

% if theta_segment_te_reinf_upper(j)<0
%     theta_segment_te_reinf_upper(j)=360 + theta_segment_te_reinf_upper(j);
% end
% 
% if theta_segment_te_reinf_lower(j)<0
%     theta_segment_te_reinf_lower(j) = 360 + theta_segment_te_reinf_lower(j);
% end


I_xx_upper_te_reinf(j)=length_element_upper_te_reinf(j)*t_eq_te_reinf^3/12; %%% As all elements are rectangles, Ixy about centroid would be zero
I_xx_lower_te_reinf(j)=length_element_lower_te_reinf(j)*t_eq_te_reinf^3/12;

I_yy_upper_te_reinf(j)=t_eq_te_reinf*length_element_upper_te_reinf(j).^3/12;
I_yy_lower_te_reinf(j)=t_eq_te_reinf*length_element_lower_te_reinf(j).^3/12;

%%%% Rotate co-ordinate system to make it parallel to chord line axis %%%%
I_xx_own_te_reinf_upper(j) = (I_xx_upper_te_reinf(j) + I_yy_upper_te_reinf(j))/2 + (I_xx_upper_te_reinf(j)-I_yy_upper_te_reinf(j)).*cosd(2*theta_segment_te_reinf_upper(j))/2;
I_xx_own_te_reinf_lower(j) = (I_xx_lower_te_reinf(j) + I_yy_lower_te_reinf(j))/2 + (I_xx_upper_te_reinf(j)-I_yy_lower_te_reinf(j)).*cosd(2*theta_segment_te_reinf_lower(j))/2;

I_yy_own_te_reinf_upper(j) = (I_xx_upper_te_reinf(j) + I_yy_upper_te_reinf(j))/2 - (I_xx_upper_te_reinf(j)-I_yy_upper_te_reinf(j)).*cosd(2*theta_segment_te_reinf_upper(j))/2;
I_yy_own_te_reinf_lower(j) = (I_xx_lower_te_reinf(j) + I_yy_lower_te_reinf(j))/2 - (I_xx_lower_te_reinf(j)-I_yy_lower_te_reinf(j)).*cosd(2*theta_segment_te_reinf_lower(j))/2;
...
theta_segment_te_panel_upper(j) = atand((airfoil_upper_y_abs(i+1)-airfoil_upper_y_abs(i))./(airfoil_upper_x_abs(i+1)-airfoil_upper_x_abs(i)));
theta_segment_te_panel_lower(j) = atand((airfoil_lower_y_abs(i+1)-airfoil_lower_y_abs(i))./(airfoil_lower_x_abs(i+1)-airfoil_lower_x_abs(i)));
% 
% if theta_segment_te_panel_upper(j)<0
%     theta_segment_te_panel_upper(j)=360 + theta_segment_te_panel_upper(j);
% end
% 
% if theta_segment_te_panel_lower(j)<0
%     theta_segment_te_panel_lower(j)=360 + theta_segment_te_panel_lower(j);
% end



I_xx_upper_te_panel(j)=length_element_upper_te(j)*t_eq_te_panel^3/12; %%% As all elements are rectangles, Ixy about centroid would be zero
I_xx_lower_te_panel(j)=length_element_lower_te(j)*t_eq_te_panel^3/12;

I_yy_upper_te_panel(j)=t_eq_te_panel*length_element_upper_te(j).^3/12;
I_yy_lower_te_panel(j)=t_eq_te_panel*length_element_lower_te(j).^3/12;

%%%% Rotate co-ordinate system to make it parallel to chord line axis %%%%
I_xx_own_te_panel_upper(j) = (I_xx_upper_te_panel(j) + I_yy_upper_te_panel(j))/2 + (I_xx_upper_te_panel(j)-I_yy_upper_te_panel(j)).*cosd(2*theta_segment_te_panel_upper(j))/2;
I_xx_own_te_panel_lower(j) = (I_xx_lower_te_panel(j) + I_yy_lower_te_panel(j))/2 + (I_xx_upper_te_panel(j)-I_yy_lower_te_panel(j)).*cosd(2*theta_segment_te_panel_lower(j))/2;

I_yy_own_te_panel_upper(j) = (I_xx_upper_te_panel(j) + I_yy_upper_te_panel(j))/2 - (I_xx_upper_te_panel(j)-I_yy_upper_te_panel(j)).*cosd(2*theta_segment_te_panel_upper(j))/2;
I_yy_own_te_panel_lower(j) = (I_xx_lower_te_panel(j) + I_yy_lower_te_panel(j))/2 - (I_xx_lower_te_panel(j)-I_yy_lower_te_panel(j)).*cosd(2*theta_segment_te_panel_lower(j))/2;
...
difference=abs(airfoil_upper_x-(shear_web1_start+(t_eq_sw/Blade.Chord(I))));
I_shear_web1_end=find(difference==min(difference));
...
difference=abs(airfoil_upper_x-(shear_web2_end-(t_eq_sw/Blade.Chord(I))));
I_shear_web2_start=find(difference==min(difference));
...
I_xx_sw1=t_eq_sw*(y_upper_shear_web1-y_lower_shear_web1)^3/12;
I_xx_sw2=t_eq_sw*(y_upper_shear_web2-y_lower_shear_web2)^3/12;

I_yy_sw1=(y_upper_shear_web1-y_lower_shear_web1)*t_eq_sw^3/12;
I_yy_sw2=(y_upper_shear_web2-y_lower_shear_web2)*t_eq_sw^3/12;
...
%%%%%% Elastic Centre of the entire section %%%%

xe_section = (sum((element_xa_upper_le + element_xa_lower_le)*E_equivalent_le_x) +...
    sum((element_xa_upper_sc + element_xa_lower_sc)*E_equivalent_sc_x) + ...
    sum((element_xa_upper_te + element_xa_lower_te)*E_equivalent_te_x) + ...
    sum((element_xa_upper_te_reinf + element_xa_lower_te_reinf)*E_equivalent_te_reinf_x)+...
    (area_shear_web1*xc_sw1 + area_shear_web2*xc_sw2)*E_equivalent_sw_x)/...
    (sum((area_element_upper_le + area_element_lower_le)*E_equivalent_le_x) + ...
    sum((area_element_upper_sc + area_element_lower_sc)*E_equivalent_sc_x) + ...
    sum((area_element_upper_te + area_element_lower_te)*E_equivalent_te_x) + ...
    sum((area_element_upper_te_reinf + area_element_lower_te_reinf)*E_equivalent_te_x)+...
    Area_equivalent_sw*E_equivalent_sw_x);

ye_section = (sum((element_ya_upper_le + element_ya_lower_le)*E_equivalent_le_x) +...
    sum((element_ya_upper_sc + element_ya_lower_sc)*E_equivalent_sc_x) + ...
    sum((element_ya_upper_te + element_ya_lower_te)*E_equivalent_te_x) + ...
    sum((element_ya_upper_te_reinf + element_ya_lower_te_reinf)*E_equivalent_te_reinf_x)+...
    (area_shear_web1*yc_sw1 + area_shear_web2*yc_sw2)*E_equivalent_sw_y)/...
    (sum((area_element_upper_le + area_element_lower_le)*E_equivalent_le_x) + ...
    (sum(area_element_upper_sc + area_element_lower_sc)*E_equivalent_sc_x) + ...
    sum((area_element_upper_te + area_element_lower_te)*E_equivalent_te_x) + ...
    sum((area_element_upper_te_reinf + area_element_lower_te_reinf)*E_equivalent_te_x)+...
...
%%%% Trasnfer I of each segment to section elastic center using Parallel
%%%% Axis Theorem %%%

I_xx_le_upper = I_xx_own_le_upper + area_element_upper_le.*((element_y_upper_le - ye_section).^2);
I_xx_le_lower = I_xx_own_le_lower + area_element_lower_le.*((element_y_lower_le - ye_section).^2);

I_xx_LE = sum(I_xx_le_upper + I_xx_le_lower); 

I_yy_le_upper = I_yy_own_le_upper + area_element_upper_le.*((element_x_upper_le - xe_section).^2);
I_yy_le_lower = I_yy_own_le_lower + area_element_lower_le.*((element_x_lower_le - xe_section).^2);
    
I_yy_LE = sum(I_yy_le_upper + I_yy_le_lower); 

I_xx_sc_upper = I_xx_own_sc_upper + area_element_upper_sc.*((element_y_upper_sc - ye_section).^2);
I_xx_sc_lower = I_xx_own_sc_lower + area_element_lower_sc.*((element_y_lower_sc - ye_section).^2);

I_xx_SC = sum(I_xx_sc_upper + I_xx_sc_lower);

I_yy_sc_upper = I_yy_own_sc_upper + area_element_upper_sc.*((element_x_upper_sc - xe_section).^2);
I_yy_sc_lower = I_yy_own_sc_lower + area_element_lower_sc.*((element_x_lower_sc - xe_section).^2);

I_yy_SC = sum(I_yy_sc_upper + I_yy_sc_lower); 

I_xx_te_reinf_upper = I_xx_own_te_reinf_upper + area_element_upper_te_reinf.*((element_y_upper_te_reinf - ye_section).^2);
I_xx_te_reinf_lower = I_xx_own_te_reinf_lower + area_element_lower_te_reinf.*((element_y_lower_te_reinf - ye_section).^2);

I_xx_TE_REINF = sum(I_xx_te_reinf_upper + I_xx_te_reinf_lower); 

I_yy_te_reinf_upper = I_yy_own_te_reinf_upper + area_element_upper_te_reinf.*((element_x_upper_te_reinf - xe_section).^2);
I_yy_te_reinf_lower = I_yy_own_te_reinf_lower + area_element_lower_te_reinf.*((element_x_lower_te_reinf - xe_section).^2);

I_yy_TE_REINF = sum(I_yy_te_reinf_upper + I_yy_te_reinf_lower); 

I_xx_te_upper = I_xx_own_te_panel_upper + area_element_upper_te.*((element_y_upper_te - ye_section).^2);
I_xx_te_lower = I_xx_own_te_panel_lower + area_element_lower_te.*((element_y_lower_te - ye_section).^2);

I_xx_TE = sum(I_xx_te_upper + I_xx_te_lower); 

I_yy_te_upper = I_yy_own_te_panel_upper + area_element_upper_te.*((element_x_upper_te - xe_section).^2);
I_yy_te_lower = I_yy_own_te_panel_lower + area_element_lower_te.*((element_x_lower_te - xe_section).^2);

I_yy_TE = sum(I_yy_te_upper + I_yy_te_lower); 

I_xx_SW=I_xx_sw1+ area_shear_web1*(yc_sw1-ye_section)^2 + ...
    I_xx_sw2 + area_shear_web2*(yc_sw2 - ye_section)^2;

I_yy_SW=I_yy_sw1+ area_shear_web1*(xc_sw1-xe_section)^2 +...
    I_yy_sw2 + area_shear_web2*(xc_sw2 - ye_section)^2;

 
rel_xe_section=xe_section/Blade.Chord(I);
rel_ye_section=ye_section/Blade.Chord(I);
...
y_edgewise_te_reinf = max(element_x_upper_te)-xe_section;
y_edgewise_skin = max(element_x_upper_te) + (t_eq_te_panel/2)-(t_triax_skin/2)-xe_section;

else

y_edgewise_te_reinf = max(element_x_upper_te_reinf)-xe_section;
y_edgewise_skin = max(element_x_upper_te_reinf) + (t_eq_te_reinf/2)-(t_triax_skin/2)-xe_section;

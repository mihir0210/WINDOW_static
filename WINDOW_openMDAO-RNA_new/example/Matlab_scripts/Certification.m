...
% load('WT_design_dynamic_opt.mat');
% load('Linearized_model_dynamic_opt.mat');

% CertificationSettings.Wind.Type=4;
% load('Control.mat');

%Control.Pitch.KiGS(2) = Control.Pitch.KiGS(1) + 0.001;



%CertificationSettings.Wind.T = 100;
...
% CertificationSettings.Wind.T=600;
% CertificationSettings.Wind.Type = 1;
% % % % %CertificationSettings.Wind.dt = 0.05;
% % CertificationSettings.Wind.Type = 1;
% CertificationSettings.Run.Time = 60;
% %
Control.DT = 0.0125;
%
%%% For fatigue %%%%
CertificationSettings.Wind.Type = 4;
CertificationSettings.Wind.T = 600;
CertificationSettings.Run.WindSpeed = [4:24];
% CertificationSettings.Run.Time = 60;
% CertificationSettings.Run.WindSpeed = 15;
%CertificationSettings.Run.Seeds =10;
...
for U = CertificationSettings.Run.WindSpeed
    ...
    for seed = 1:CertificationSettings.Run.Seeds
        ...
        fprintf('Done before InflowWind\n');
        InflowWind_constantseed(CertificationSettings.Wind,U,Tower.HubHeight,Blade.Radius(end))
        ...
        evalc('sim(''FAST'',TMax);');
        %%% Initialize loop for geenrating wind files and running FAST
        %%% IF it encounters an error, it will keep on iterating
        %%% While loop is useful when different seeds are used everytime
%         trying = true;
%         attemptCount = 1;
%         maxAttempts = 5;
%         while trying
...
%                 
%             catch
%                 fprintf('Failed in attempt no. (%d) due to an error in inflow angle, Retrying...\n', attemptCount);
%                 
%                 if attemptCount > maxAttempts
%                     error('Function failed : Max number of attempts reached');
%                 end
...
%         end
        ...
        %
        %
        %             fprintf('Failed in attempt no. 1 due to an error in inflow angle, Retrying...\n');
        %             % Wind input file
        %             disp('Generating wind file...')
        %
        %             %Control.DT = 0.008;
        ...
        %
        %             % Set operation mode in ElastoDyn file
        %             ElastoDyn_new(Blade,Tower,Nacelle,Drivetrain,Control,CertificationSettings.Mode.Type,RPM_Init,P_InitAngle);
        %             ServoDyn(Drivetrain,Control,CertificationSettings.Mode.Type,CertificationSettings.Mode.Actiontime);
        %
        %             InflowWind_constantseed(CertificationSettings.Wind,U,Tower.HubHeight,Blade.Radius(end))
        %
...
%fclose('all'); %to prevent Matlab IO error due to exhaustion of all file IDs

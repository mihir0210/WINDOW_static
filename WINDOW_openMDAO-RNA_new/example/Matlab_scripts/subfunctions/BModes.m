...
if type == 1
    ...
    %props = [pwd, '\inputfiles\BModes_blade.dat'];
...
if type == 2
   ...
   % props = [pwd, '\inputfiles\BModes_tower.dat'];
...
%props_1 = [pwd, '\inputfiles\BModes.bmi'];
%fid = fopen(props_1, 'wt');
...
%system(['\bmodes "', pwd, '\inputfiles\BModes.bmi"']);

%% Find modes
% Read output
 fileID = fopen([pwd, '\subfunctions\inputfiles\BModes.out'],'r');
%fileID = fopen([pwd, '\inputfiles\BModes.out'],'r');

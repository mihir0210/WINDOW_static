function ElastoDyn_new(Blade,Tower,Nacelle,Drivetrain,Control,mode,varargin)
...
fprintf(fid, '%s         FlapDOF1    - First flapwise blade mode DOF (flag)\n',FlapDOF1);
fprintf(fid, '%s          FlapDOF2    - Second flapwise blade mode DOF (flag)\n',FlapDOF2);
fprintf(fid, '%s          EdgeDOF     - First edgewise blade mode DOF (flag)\n',EdgeDOF);
fprintf(fid, 'False         TeetDOF     - Rotor-teeter DOF (flag) [unused for 3 blades]\n');
fprintf(fid, '%s          DrTrDOF     - Drivetrain rotational-flexibility DOF (flag)\n',DrTrDOF);
fprintf(fid, '%s          GenDOF      - Generator DOF (flag)\n', GenDOF);
fprintf(fid, '%s          YawDOF      - Yaw DOF (flag)\n', YawDOF);
fprintf(fid, '%s          TwFADOF1    - First fore-aft tower bending-mode DOF (flag)\n',TwFADOF1);
fprintf(fid, '%s          TwFADOF2    - Second fore-aft tower bending-mode DOF (flag)\n',TwFADOF2);
fprintf(fid, '%s          TwSSDOF1    - First side-to-side tower bending-mode DOF (flag)\n',TwSSDOF1);
fprintf(fid, '%s          TwSSDOF2    - Second side-to-side tower bending-mode DOF (flag)\n',TwSSDOF2);
...
fprintf(fid, '          4   OoPDefl     - Initial out-of-plane blade-tip displacement (meters)\n');
...
fprintf(fid, '---------------------- BLADE ---------------------------------------------------\n');
fprintf(fid, '         49   BldNodes    - Number of blade nodes (per blade) used for analysis (-)\n');
...
fprintf(fid, '         %i   NBlGages    - Number of blade nodes that have strain gages for output [0 to 9] (-)\n', NBlGages);
fprintf(fid, '          %i, %i, %i, %i, %i             BldGagNd    - List of blade nodes that have strain gages [1 to BldNodes] (-) [unused if NBlGages=0]\n', Nodes);
...
fprintf(fid, '"Spn1MLxb1"               - Blade 1 local edgewise bending moments at span station 1 (approx. 15%% span)\n');
fprintf(fid, '"Spn1MLyb1"               - Blade 1 local flapwise bending moments at span station 1 (approx. 15%% span)\n');
fprintf(fid, '"Spn2MLxb1"               - Blade 1 local edgewise bending moments at span station 2 (approx. 30%% span)\n');
fprintf(fid, '"Spn2MLyb1"               - Blade 1 local flapwise bending moments at span station 2 (approx. 30%% span)\n');
fprintf(fid, '"Spn3MLxb1"               - Blade 1 local edgewise bending moments at span station 3 (approx. 50%% span)\n');
fprintf(fid, '"Spn3MLyb1"               - Blade 1 local flapwise bending moments at span station 3 (approx. 50%% span)\n');
fprintf(fid, '"Spn4MLxb1"               - Blade 1 local edgewise bending moments at span station 4 (approx. 75%% span)\n');
fprintf(fid, '"Spn4MLyb1"               - Blade 1 local flapwise bending moments at span station 4 (approx. 75%% span)\n');
fprintf(fid, '"Spn5MLxb1"               - Blade 1 local edgewise bending moments at span station 5 (approx. 95%% span)\n');
fprintf(fid, '"Spn5MLyb1"               - Blade 1 local flapwise bending moments at span station 5 (approx. 95%% span)\n');

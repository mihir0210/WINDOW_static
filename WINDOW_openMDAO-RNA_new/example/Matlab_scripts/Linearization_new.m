%% Set file names9
function LinModel=Linearization_new(rated_windspeed,Airfoil,Blade,Control,Drivetrain,Nacelle,Tower)
...
if Lin.Pitch(I)==Lin.Pitch(I+1)
    x = [WindSpeeds(I), WindSpeeds(I+2)];
    pitch = [Lin.Pitch(I), Lin.Pitch(I+2)];
    x_new = WindSpeeds(I:I+2);
    pitch_new = interp1(x,pitch,x_new);
    Lin.Pitch(I+1)=pitch_new(2);
end

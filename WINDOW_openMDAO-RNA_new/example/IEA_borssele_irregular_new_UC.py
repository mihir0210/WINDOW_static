...
options.input.turbine.wind_speed_file = 'Input/NL_wind_speed.csv'
options.input.turbine.spot_price_file = 'Input/NL_spot_price.csv'
...
problem.driver.options['bits'] = {'design_tsr':5, 'chord_coefficients':5 ,
                                    'twist_coefficients':5, 'pitch': 5, 'tau_root':5, 'tau_75':5}
problem.driver.options['max_gen'] = 15
...
recorder = SqliteRecorder("dynamic_SLSQP.csv")
...
#problem.driver.options['rhobeg'] = 1.0
#problem.driver.options['rhoend'] = 1.0e-4
#problem.driver.options['maxfun']  = 100
...
problem['indep2.design_tsr'] = 7.6 #7.6 but 1 as it will be multiplied by reference
problem['indep2.blade_number'] = 3
problem['indep2.chord_coefficients'] = np.array([3.542, 3.01, 2.313])  #* 190.8 / 126.0  #np.array([3.35, 3.5, 2.6])
problem['indep2.twist_coefficients'] = np.array([13.308, 9.0, 3.125])  #np.array([9.1, 4.3, 1.8])
problem['indep2.span_airfoil_r'] = np.array([01.36, 06.83, 10.25, 14.35, 22.55, 26.65, 34.85, 43.05]) * 170/ 126.0
problem['indep2.span_airfoil_id'] = [0, 1, 2, 3, 4, 5, 6, 7]
problem['indep2.pitch'] = 0
...
problem.model.add_design_var('design_tsr', lower=0.855, upper=1.118)
problem.model.add_design_var('chord_coefficients', lower=[0.75, 0.75,0.75], upper=[1.25,1.25,1.25])
problem.model.add_design_var('twist_coefficients', lower=0.7 * np.ones(3), upper=1.3 * np.ones(3))
problem.model.add_design_var('pitch', lower=-0.028, upper=1)
#problem.model.add_design_var('tau_root', lower=0.7, upper=1.3)
#problem.model.add_design_var('tau_75', lower=0.7, upper=1.3)
problem.model.add_design_var('tau', lower=0.7, upper=1.3)
problem.model.add_objective('obj.f')  # ref0=-1.5*f_scaler, ref=-0.5*f_scaler)
problem.model.add_constraint('c1.tip_deflection', upper = 1)  # , ref0=0.25, ref=2.0)
problem.model.add_constraint('c2.Spar_stress', upper = 1)
problem.model.add_constraint('c3.Skin_stress', upper = 1)
problem.model.add_constraint('c4.Te_Reinf_stress', upper = 1)'''

'''
#problem.model.add_design_var('machine_rating', lower=3000, upper=6000)

problem.model.add_design_var('turbine_radius', lower=50, upper=150)
...
problem.run_model()  # runs model once
#problem.model.approx_totals('fd')
#problem.run_driver() # use this for optimization and not run_model
